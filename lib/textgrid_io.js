...
function serializeTextgridToCsv (tg, pivotTierName, tierNameList = null, includeHeader = true) {
  ...
  for (let i = 0; i < tier.entryList.length; i++) {
    ...
    let subTG = cropTextgrid(tg, start, stop, 'truncated', false);
 ...
 */
function serializeTextgrid (tg, minimumIntervalLength = MIN_INTERVAL_LENGTH, minTimestamp = null, maxTimestamp = null) {
  if (minTimestamp === null) minTimestamp = tg.minTimestamp;
  if (maxTimestamp === null) maxTimestamp = tg.maxTimestamp;

  let outputTg = prepTgForSaving(tg, minimumIntervalLength, minTimestamp, maxTimestamp);
  ...
  outputTxt += `<exists>\n${outputTg.tierNameList.length}\n`;

  for (let i = 0; i < outputTg.tierNameList.length; i++) {
    outputTxt += tierToText(outputTg.tierDict[outputTg.tierNameList[i]]);
  }
